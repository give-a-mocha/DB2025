...
std::shared_ptr<Plan> Planner::generate_group_and_aggregate_plan(std::shared_ptr<Query> query,
    ...
    return std::make_shared<GroupAggregatePlan>(PlanTag::T_GroupAggregate, std::move(plan),
                                                std::move(query->group_cols), std::move(agg_cols),
...
std::shared_ptr<Plan> Planner::generate_select_plan(std::shared_ptr<Query> query, Context *context) {
    ...
    if (is_star || is_count_star) {
        return std::make_shared<ProjectionPlan>(PlanTag::T_Projection, std::move(plannerRoot), std::move(sel_cols),
                                                is_star);
    }
    // 保证投影列不重复
...
std::shared_ptr<Plan> Planner::do_planner(std::shared_ptr<Query> query, Context *context) {
    ...
    switch (query->parse->getType()) {
        ...
        case ast::AstType::CreateIndex: {
            ...
            // create index;
            plannerRoot = std::make_shared<DDLPlan>(PlanTag::T_CreateIndex, std::move(x->tab_name),
                                                    std::move(x->col_names), std::vector<ColDef>());
            break;
        }
        case ast::AstType::DropIndex: {
            auto x = std::static_pointer_cast<ast::DropIndex>(query->parse);
            // drop index
            plannerRoot = std::make_shared<DDLPlan>(PlanTag::T_DropIndex, std::move(x->tab_name),
                                                    std::move(x->col_names), std::vector<ColDef>());
            break;
        }
        case ast::AstType::InsertStmt: {
            ...
            // insert;
            plannerRoot =
                std::make_shared<DMLPlan>(PlanTag::T_Insert, std::shared_ptr<Plan>(), x->tab_name,
        ...
        case ast::AstType::DeleteStmt: {
            ...
            if (index_exist == false) {  // 该表没有索引
                index_col_names.clear();
                table_scan_executors = std::make_shared<ScanPlan>(PlanTag::T_SeqScan, x->tab_name,
                                                                  std::move(query->conds), std::move(index_col_names));
            } else {  // 存在索引
                table_scan_executors = std::make_shared<ScanPlan>(PlanTag::T_IndexScan, x->tab_name,
                                                                  std::move(query->conds), std::move(index_col_names));
            }

            plannerRoot =
                std::make_shared<DMLPlan>(PlanTag::T_Delete, std::move(table_scan_executors), x->tab_name,
        ...
        case ast::AstType::UpdateStmt: {
            ...
            plannerRoot = std::make_unique<DMLPlan>(PlanTag::T_Update, std::move(table_scan_executors),
                                                    x->tab_name->tab_name, std::vector<Value>(),
                                                    std::vector<Condition>(), std::move(query->set_clauses));
            break;
        }
