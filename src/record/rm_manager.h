...
class RmManager {
   ...
   public:
    ...
    void create_file(const std::string &filename, int record_size) {
        ...
        // [RmPageHdr] [Bitmap] [Bitmap] [Record1] [Record2] ... [RecordN]
        // sizeof(RmPageHdr) + bitmap_size + (记录数 × record_size) ≤ PAGE_SIZE
        // sizeof(hdr) + (n + BITMAP_WIDTH - 1) / BITMAP_WIDTH * 2 + n * record_size <= PAGE_SIZE
        file_hdr.num_records_per_page =
            (BITMAP_WIDTH * (PAGE_SIZE - 2 - (int)sizeof(RmPageHdr)) + 2) / (2 + record_size * BITMAP_WIDTH);
