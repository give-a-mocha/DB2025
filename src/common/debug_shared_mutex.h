...
#ifdef DEBUG_LOCKS
        INFO("[LOCK_SHARED] Thread {} acquired shared lock on {} (count: {})", current_thread, name_,
             shared_count_.load());
#endif
    }
...
#ifdef DEBUG_LOCKS
        INFO("[UNLOCK_SHARED] Thread {} releasing shared lock on {} (count before: {})", current_thread, name_,
             shared_count_.load());
#endif
...
#ifdef DEBUG_LOCKS
        INFO("[UNLOCK_SHARED] Thread {} released shared lock on {} (count: {})", current_thread, name_,
             shared_count_.load());
#endif
    }
...
#ifdef DEBUG_LOCKS
            INFO("[TRY_LOCK_SHARED] Thread {} acquired shared lock on {} (count: {})", current_thread, name_,
                 shared_count_.load());
#endif
            return true;
        }
...
// 便利宏定义
#ifdef DEBUG_LOCKS
#define DEBUG_MUTEX_STATE(mutex) \
    do {                         \
        (mutex).print_state();   \
    } while (0)
#define DEBUG_MUTEX_INFO(mutex, msg)                              \
    do {                                                          \
